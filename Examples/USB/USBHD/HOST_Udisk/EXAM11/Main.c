			   ...
			   if ( CH103DiskStatus >= DISK_MOUNTED )
                ...
                /* 读文件 */
                    ...
                    for ( j = 0; j < 10000; j ++ )                                 //限定10000个文件,实际上没有限制
                    {
                        ...
                        mCmdParam.Open.mPathName[ i ] = 0xFF;                      //根据字符串长度将结束符替换为搜索的序号,从0到254,如果是0xFF即255则说明搜索序号在CH103vFileSize变量中
                        CH103vFileSize = j;                                        //指定搜索/枚举的序号
                        i = CH103FileOpen( );                                      //打开文件,如果文件名中含有通配符*,则为搜索文件而不打开
                        /* CH103FileEnum 与 CH103FileOpen 的唯一区别是当后者返回ERR_FOUND_NAME时那么对应于前者返回ERR_SUCCESS */
                        if ( i == ERR_MISS_FILE )
                        {
